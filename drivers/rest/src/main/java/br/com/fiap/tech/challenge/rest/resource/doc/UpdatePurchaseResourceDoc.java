...
public interface UpdatePurchaseResourceDoc {
    ...
    @Operation(
            ...
            responses = {
                    @ApiResponse(responseCode = "200", description = "OK - Retorno em caso de sucesso na atualização do pedido", content = { @Content(schema = @Schema(implementation = PurchaseResponse.class), mediaType = "application/json") }),
                    @ApiResponse(responseCode = "400", description = "O UUID ou o status do pedido são inválidos", content = { @Content(schema = @Schema()) })
            }
    )
    PurchaseResponse updateStatus(@Parameter(description = "UUID do pedido a ser atualizado", required = true) String uuid,
