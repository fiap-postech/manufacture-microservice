...
public interface QueryPurchaseResourceDoc {
    ...
    @Operation(
            ...
            responses = {
                    @ApiResponse(responseCode = "200", description = "OK - Retorno em caso de sucesso na solicitação de busca de pedidos por status", content = { @Content(schema = @Schema(implementation = PurchaseResponse.class), mediaType = "application/json") }),
                    @ApiResponse(responseCode = "400", description = "O status do pedido fornecido não é válido", content = { @Content(schema = @Schema()) })
            }
    )
    List<PurchaseResponse> getByStatus(@Parameter(description = "Status dos pedidos a serem pesquisados", required = true) PurchaseStatus status);
}